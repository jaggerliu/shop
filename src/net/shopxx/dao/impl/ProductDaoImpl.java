...
public class ProductDaoImpl extends BaseDaoImpl<Product, Long> implements ProductDao {
	...
	public List<Product> findByUse(ProductUse productUse) {
		...
		String jpql = "select product from Product product where lower(product.productUse) = lower(:productUseId)";
		try {
			return entityManager.createQuery(jpql, Product.class).setFlushMode(FlushModeType.COMMIT).setParameter("productUseId", productUse.getId()).getResultList();
		} catch (NoResultException e) {
			return null;
		}
	}
	...
	public List<Product> findList(ProductCategory productCategory,ProductUse productUse, Brand brand, Promotion promotion, List<Tag> tags, Map<Attribute, String> attributeValue, BigDecimal startPrice, BigDecimal endPrice, Boolean isMarketable, Boolean isList, Boolean isTop, Boolean isGift, Boolean isOutOfStock, Boolean isStockAlert, OrderType orderType, Integer count, List<Filter> filters, List<Order> orders) {
		...
		if (productUse != null) {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("productUse"), productUse));
		}
		if (promotion != null) {
			Subquery<Product> subquery1 = criteriaQuery.subquery(Product.class);
			Root<Product> subqueryRoot1 = subquery1.from(Product.class);
